...
public class FragmentLikedUser extends BaseHomeFragment {
    ...
    private Call<LikedUserList> likedUserCall;
    ...
    private void getLikedUsersOfPost(final int page) {
        likedUserCall = ApiCallingService.Posts.getLikedUsers(id, page, context);
        if (!likedUserCall.isExecuted()) likedUserCall.enqueue(likedUserCallback(page));
    }

    private void getLikedUsersOfReaction(int page) {
        likedUserCall = ApiCallingService.React.getLikedUsersReaction(id, page, context);
        if (!likedUserCall.isExecuted()) likedUserCall.enqueue(likedUserCallback(page));
    }
    ...
    private void getMyProfileLikedUsers(int page) {
        likedUserCall = ApiCallingService.User.getLikedUserProfile(page, context);
        if (!likedUserCall.isExecuted()) likedUserCall.enqueue(likedUserCallback(page));
    }

    private void getOtherProfileLikedUser(int page) {
        likedUserCall = ApiCallingService.Friends.getLikedUserFriendProfile(id, page, context);
        if (!likedUserCall.isExecuted()) likedUserCall.enqueue(likedUserCallback(page));
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        if (likedUserCall != null) likedUserCall.cancel();
    }
}