...
import static com.cncoding.teazer.utilities.common.SharedPrefs.setFollowingNotificationCount;

/**
 * A fragment representing a list of Items.
 */
public class FollowingNotificationsTabFragment extends BaseHomeFragment {
...
//    private OnListFragmentInteractionListener mListener;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        LinearLayoutManager manager = new LinearLayoutManager(getTheContext());
    ...
    private void getFollowingNotifications(final int page) {
        notificationsListCall = ApiCallingService.User.getFollowingNotifications(page, getTheContext());
                        ...
                        if (isAdded()) {
                            if (response.code() == 200) {
                                ...
                                if (response.body().getNotifications().size() > 0) {
                                    if (page == 1) {
                                        notificationsList.getNotifications().clear();
                                    }
                                ...
                                Toast.makeText(getTheContext(), response.code() + " : " + response.message(),
    ...
    private void resetFollowingNotification() {
        resetNotificationCall = ApiCallingService.User.resetUnreadNotification(getTheContext(), 1);
            ...
            resetNotificationCall.enqueue(new Callback<ResultObject>() {
                ...
                public void onResponse(Call<ResultObject> call, Response<ResultObject> response) {
                    try {
                        if (isAdded()) {
                            if (response.code() == 200) {
                                setFollowingNotificationCount(getTheContext() ,0);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
