...
import static com.cncoding.teazer.utilities.common.SharedPrefs.setRequestNotificationCount;
...
//    private OnListFragmentInteractionListener mListener;
    ...
    private void getRequestNotifications(final int page) {
        ...
        notificationsListCall = ApiCallingService.User.getRequestNotifications(page, getTheContext());
...
//    }
    ...
    private void resetRequestNotification() {
        resetNotificationCall = ApiCallingService.User.resetUnreadNotification(getTheContext(), 2);
            ...
            resetNotificationCall.enqueue(new Callback<ResultObject>() {
                ...
                public void onResponse(Call<ResultObject> call, Response<ResultObject> response) {
                    try {
                        if (isAdded()) {
                            if (response.code() == 200) {
                                setRequestNotificationCount(getTheContext() ,0);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
