...
public interface ToroPlayer {
    ...
    /**
     * Initialize resource for the incoming playback. After this point, {@link im.ene.toro.ToroPlayer} should be
     * able to start the playback at anytime in the future (This doesn't mean that any call to {@link
     * im.ene.toro.ToroPlayer#play()} will start the playback immediately. It can start buffering enough resource
    ...
    /**
     * A convenient callback to help {@link im.ene.toro.ToroPlayer} to listen to different playback states.
     */
    ...
    @IntDef({ im.ene.toro.ToroPlayer.State.STATE_IDLE, im.ene.toro.ToroPlayer.State.STATE_BUFFERING, im.ene.toro.ToroPlayer.State.STATE_READY, im.ene.toro.ToroPlayer.State.STATE_END })  //
