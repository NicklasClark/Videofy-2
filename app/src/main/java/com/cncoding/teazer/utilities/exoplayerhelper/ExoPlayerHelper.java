        ...
        public void onTracksChanged(TrackGroupArray trackGroups, TrackSelectionArray trackSelections) {
            ...
            if (mappedTrackInfo != null) {
                if (mappedTrackInfo.getTrackTypeRendererSupport(C.TRACK_TYPE_VIDEO)
                    ...
                    Toast.makeText(context, im.ene.toro.R.string.error_unsupported_video, Toast.LENGTH_SHORT).show();
                }
                if (mappedTrackInfo.getTrackTypeRendererSupport(C.TRACK_TYPE_AUDIO)
                        == MappingTrackSelector.MappedTrackInfo.RENDERER_SUPPORT_UNSUPPORTED_TRACKS) {
                    Toast.makeText(context, im.ene.toro.R.string.error_unsupported_audio, Toast.LENGTH_SHORT).show();
                }
            }
            int count;
            if (eventListeners != null && (count = eventListeners.size()) > 0) {
